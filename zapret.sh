...
ensure_unzip_tool() {
    ...
    if busybox 2>/dev/null | grep -qw unzip; then
        UNZIP_CMD="busybox unzip"
        return 0
    fi

    log "${CYAN}unzip не найден, пробую установить${NC}"

    if [ "$PKG_IS_APK" -eq 1 ]; then
        run_with_timeout 40 apk update || true
        run_with_timeout 60 apk add unzip || fail "Не удалось установить unzip"
    else
        run_with_timeout 40 opkg update || true
        run_with_timeout 60 opkg install unzip || fail "Не удалось установить unzip"
    fi
    ...
    fail "Не удалось подготовить unzip"
}
...
extract_archive() {
    ...
    sh -c "$UNZIP_CMD -o \"$FILE_NAME\"" >/dev/null 2>&1 || fail "Не удалось распаковать архив"
}
